    ...
    <>
      {isAuthenticated && (
        <div className='grid grid-cols-5 grid-rows-5 gap-4 h-screen'>
          ...
          <div className={`col-span-3 row-span-4 col-start-2 row-start-2 border overflow-scroll transition ${isModalOpen ? 'opacity-50 pointer-events-none' : ''}`} id='main_container'>
            <TwitterLikeFeed />
          </div>
          <button className='absolute bottom-4 right-4 bg-blue-500 text-white px-4 py-2 rounded shadow' onClick={openPostForm}>
            New Post
          </button>
          {isModalOpen && (
            <div
