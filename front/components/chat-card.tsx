            ...
            <div key={message.id} className='flex items-start gap-3'>
              ...
              <div className='flex-1 min-w-0'>
                <div className='flex items-center gap-2 mb-1'>
                  <span className={cn('font-medium', isLightTheme ? 'text-zinc-900' : 'text-zinc-100')}>{message.sender.name}</span>
                  <span className={cn('text-sm', isLightTheme ? 'text-zinc-500' : 'text-zinc-500')}>{message.timestamp}</span>
                </div>
                <p className={cn('break-words', isLightTheme ? 'text-zinc-700' : 'text-zinc-300')}>{message.content}</p>
                {message.reactions && message.reactions.length > 0 && (
                  <div className='flex items-center gap-1 mt-2'>
                    {message.reactions.map((reaction) => (
                      <button key={reaction.emoji} onClick={() => handleReaction(message.id, reaction.emoji)} className={cn('px-2 py-1 rounded-lg text-sm flex items-center gap-1', reaction.reacted ? (isLightTheme ? 'bg-violet-100 text-violet-600' : 'bg-violet-500/20 text-violet-400') : isLightTheme ? 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200' : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700')}>
              ...
              <div className='flex items-center self-end mb-1'>
                ...
                {message.status === 'delivered' && <Check className={cn('w-4 h-4', isLightTheme ? 'text-zinc-400' : 'text-zinc-500')} />}
              </div>
            </div>
          ))}
        </div>
